...
async function ensureCampaignPublicColumns(db) {
  ...
  if (!existing.has('promoted_until')) {
    await db.exec(`ALTER TABLE campaigns ADD COLUMN promoted_until TEXT`);
  }

  if (!existing.has('is_public')) {
    await db.exec(`ALTER TABLE campaigns ADD COLUMN is_public INTEGER NOT NULL DEFAULT 1`);
  }
}
